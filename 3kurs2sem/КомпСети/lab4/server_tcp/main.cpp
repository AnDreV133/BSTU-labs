...
void* ping_message_task(void* arg)
{
    ...
    hostent* serv_net = gethostbyname(host_name); 

    unsigned char ip[4] = { 
        ((struct in_addr*)(serv_net->h_addr))->S_un.S_un_b.s_b1, 
        ((struct in_addr*)(serv_net->h_addr))->S_un.S_un_b.s_b2, 
        ((struct in_addr*)(serv_net->h_addr))->S_un.S_un_b.s_b3, 
        ((struct in_addr*)(serv_net->h_addr))->S_un.S_un_b.s_b4 
    }; 
    ...
    serv_info.sin_addr.S_un.S_un_b.s_b1 = ip[0]; 
    serv_info.sin_addr.S_un.S_un_b.s_b2 = ip[1]; 
    serv_info.sin_addr.S_un.S_un_b.s_b3 = ip[2]; 
    serv_info.sin_addr.S_un.S_un_b.s_b4 = ip[3]; 
    serv_info.sin_port = htons(1234); 

    if (bind(socket_descriptor, (sockaddr *)&serv_info, sizeof(serv_info)))
    {
        printf("bind failed with error: %d\n", WSAGetLastError());
        return nullptr;
    }

    int namelen = sizeof(serv_info);
    getsockname(socket_descriptor, (sockaddr *)(&serv_info), &namelen);
    ...
    char accept_client_data[CLIENT_REQUEST_SIZE];

...
int WINAPI WinMain(HINSTANCE hThisInstance, HINSTANCE hPrevInst, LPSTR lpszArgs, int nWinMode) {
    ...
    hWndStatusBar = CreateWindowExA(
            0, STATUSCLASSNAMEA, NULL,
             WS_VISIBLE | SBARS_SIZEGRIP | WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN,
            0, 0, 0, 0,
            hWnd, (HMENU) 10001,
            hThisInstance, NULL
    );
...
//            break;
        ...
        case WM_KEYDOWN:
            ...
            if (wParam == VK_LEFT) { x_trsl -= TRSL_FIGURE; }
            if (wParam == VK_RIGHT) { x_trsl += TRSL_FIGURE; }
            if (wParam == VK_UP) { y_trsl += TRSL_FIGURE; }
            if (wParam == VK_DOWN) { y_trsl -= TRSL_FIGURE; }
            if (wParam == VK_OEM_4) { global_alpha -= 5; }
            if (wParam == VK_OEM_6) { global_alpha += 5; }
        ...
        case WM_MOUSEMOVE:
            if (lmbPressed) {
                ...
                if (lmbFirst) {
                    ...
                    y_rot += (xMouseMove - prevMouseX) / 30.0;
                    x_rot += (yMouseMove - prevMouseY) / 30.0;
                    prevMouseX = xMouseMove;
                    prevMouseY = yMouseMove;
                }
            }

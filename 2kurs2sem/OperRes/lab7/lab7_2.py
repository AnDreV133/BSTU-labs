...
def method_branch_and_bound(A, b, c, bounds):
    ...
    for index, x in zip(range(len(res.x)), res.x):
        if not is_int(x):
            # bounds_for_bigger_than = [(0, None) if index != i else (m.ceil(x), None) for i in range(len(bounds))]
            # bounds_for_less_than = [(0, None) if index != i else (0, m.floor(x)) for i in range(len(bounds))]
